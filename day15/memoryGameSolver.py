...
def nextNumberSpokenQuick(numberDictionary, lastNumber):

    # If the last number is spoken more than once, next number is the number of turns apart the number is from when it was previously spoken.
    if numberDictionary[lastNumber][0] > 0:
        return numberDictionary[lastNumber][1] - numberDictionary[lastNumber][0]
...
numberDictionary = {6:(-1,1), 13:(-1,2), 1:(-1,3), 15:(-1,4), 2:(-1,5), 0:(-1,6)}

nextNumber = 0
for turn in range(7, 30000001):
    nextNumber = nextNumberSpokenQuick(numberDictionary, nextNumber)
